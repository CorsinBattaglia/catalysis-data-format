...
__all__ = ["CDFValidationError", "validate_df"]

class CDFValidationError(Exception):
    """Raised when a DataFrame fails CDF validation."""
...
def _print_report(rep: Dict[str, Any]) -> None:
    ...
    if ts.get("present") and not ts.get("monotonic", True):
        ...
        print("      Suggestion: cdf.clean(df, time_fix='segment') or cdf.repair.fix_time(df, method='auto').")
    ...
    *,
    ...
    if ts.get("present") and not ts.get("monotonic", True):
        warnings.warn(
            ...
            f"(min Δ = {ts['min_drop']:.6g} s). Consider cdf.repair.fix_time(...).",
            RuntimeWarning,
            stacklevel=2,
        )
    ...
    if raise_on_error and not rep["ok"]:
        raise CDFValidationError(f"Missing required columns: {rep['missing']}")
