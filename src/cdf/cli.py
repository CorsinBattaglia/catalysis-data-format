...
def validate(path: str, strict: bool = typer.Option(False, help="Raise error (non-zero exit) on invalid BDF"),
    ...
    try:
    ...
    except CDFValidationError as e:
        if json:
        ...
        print(f"[cdf] Error reading file: {e}")
...
def templates(
    ...
    for p in created:
        print(f"[cdf] created {p}")
    for p in skipped:
        print(f"[cdf] skipped {p}")
...
def plot(
    ...
    print(f"[cdf] plotted {', '.join(ydata)} vs {xdata}" + (f" -> {save}" if save else ""))
