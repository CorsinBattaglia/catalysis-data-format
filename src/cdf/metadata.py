...
try:
    from cdf.units import resolve_unit  # type: ignore
except Exception:
    def resolve_unit(value: Any, *, as_string: bool = False):  # fallback stub
        raise RuntimeError("cdf.units.resolve_unit is unavailable")
...
class Battery:
    ...
    def to_schemaorg(self) -> Dict[str, Any]:
        ...
        _add_schema_prop("IEC code", "cdf:iec_code", self.iec_code)
        _add_schema_prop("Chemistry", "cdf:chemistry", self.chemistry)
        _add_schema_prop("Form factor", "cdf:form_factor", self.form_factor)
        _add_schema_prop("Nominal voltage", "cdf:nominal_voltage_v", self.nominal_voltage_v, "V")
        _add_schema_prop("Rated capacity", "cdf:rated_capacity_ah", self.rated_capacity_ah, "Ah")
        _add_schema_prop("Rated energy", "cdf:rated_energy_wh", self.rated_energy_wh, "Wh")
        _add_schema_prop("Mass", "cdf:mass_g", self.mass_g, "g")
        _add_schema_prop("Volume", "cdf:volume_l", self.volume_l, "L")
...
def _ensure_spec():
    ...
    if _SPEC is None:
        try:
            from cdf.normalize import spec as _loaded  # type: ignore
            _SPEC = _loaded
        except Exception:
            _SPEC = None
...
def _variable_measured_from_df(df: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    Build a list of schema.org PropertyValue from DataFrame columns:
      - Prefer df.attrs["cdf:columns"] for quantity+unit if present
      - Else, derive name from column label left side; unit via cdf.units.resolve_unit
    ...
    meta = getattr(df, "attrs", {}).get("cdf:columns", {})
