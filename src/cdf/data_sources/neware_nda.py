...
class _NewareNDABase(CyclerPlugin):
    ...
    column_synonyms = {
        ...
        "Step Count / 1": ["Step", "step_count"],
        "Step Index / 1": ["Step_Index", "step_index"],
    ...
    def fixup(self, df: pd.DataFrame) -> pd.DataFrame:
        out = df
        meta = out.attrs.get("cdf:columns", {}) if hasattr(out, "attrs") else {}
    ...
    def parse(self, path: Path) -> pd.DataFrame:
        try:
            ...
            try:
                ...
                raise ImportError(
                    "Reading .nda/.ndax requires NewareNDA. Install catalysisdf "
