    ...
    def parse(self, path: Path) -> pd.DataFrame:
        ...
        if mapping is None:
            raise ValueError(
                ...
                "Create a sidecar <name>.map.json or cdf.mapping.json."
            )
        ...
        if dt_cfg:
            ...
            if "cdf:timestamp" not in df.attrs:
                df.attrs["cdf:timestamp"] = {
                    "column": dt_target,
        ...
        for col, unit in units.items():
            if col not in df.columns:
                continue
            try:
                from cdf.units.core import convert_series, resolve_unit
...
def _load_mapping(path: Path) -> Optional[dict[str, Any]]:
    ...
    candidates = [
        ...
        path.with_name("cdf.mapping.json"),
        path.with_name("cdf.map.json"),
    ]
...
def _normalize_mapping(mapping: dict[str, Any]) -> dict[str, Any]:
    def _looks_like_bdf(label: str) -> bool:
        ...
        if text.lower() in {
            ...
            return True
        try:
            from cdf.normalize import spec
