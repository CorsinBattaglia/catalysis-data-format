...
class DelimitedTextPlugin(CyclerPlugin):
    ...
    def parse(self, path: Path) -> pd.DataFrame:
        ...
        if self.ragged_row_policy == "fold_last":
            ...
            if not used_marker and not self._looks_ok(df) and header_idx > 0:
                if self._debug_on():
                    print(f"[cdf][DelimitedTextPlugin] retry one line up: header_idx={header_idx-1}")
        ...
        if self._debug_on():
            print(f"[cdf][DelimitedTextPlugin] columns={list(df.columns)}")
    ...
    def _find_header_from_marker(self, path: Path, enc: str, marker_regex: str, offset: int) -> tuple[int, str]:
        ...
        if self._debug_on():
            print(f"[cdf][DelimitedTextPlugin] marker={marker_regex!r} header_idx={header_idx} sep={sep!r} header_line={header_line!r}")
