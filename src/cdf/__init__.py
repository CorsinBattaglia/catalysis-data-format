...
from .validate import CDFValidationError, validate_df  # prints report if asked; warns on non-monotonic time
...
try:
    ...
    try:
        __version__ = _pkg_version("catalysisdf")
    except Exception:
        __version__ = _pkg_version("cdf")
except Exception:
...
def _cdf_short_formatwarning(message, category, filename, lineno, line=None):
    """
    Render warnings without absolute paths. If the warning originates inside
    the cdf package, just show 'cdf.<module>:<lineno>'; otherwise show a short
    filename. Message text remains unchanged.
    """
    try:
        p = Path(filename).resolve()
        # Heuristic: if file path contains '/cdf/' (or '\cdf\') treat it as our package
        fp = str(p).replace("\\", "/")
        if "/cdf/" in fp or fp.endswith("/cdf/__init__.py"):
            # Build a dotted module-ish label
            try:
                # relative to the package root
                pkg_root = Path(__file__).resolve().parent
                rel = p.relative_to(pkg_root)
                mod = "cdf." + ".".join(rel.with_suffix("").parts)
            except Exception:
                mod = "cdf"
            where = f"{mod}:{lineno}"
        else:
...
def _enable_short_warnings() -> bool:
    val = os.getenv("CDF_FORMAT_WARNINGS", "").strip().lower()
    return val in {"1", "true", "yes", "on"}


# Install the formatter (opt-in via env var).
if _enable_short_warnings():
    warnings.formatwarning = _cdf_short_formatwarning
...
def _default_ingest_cache_dir() -> Path:
    ...
    env = os.getenv("CDF_CRAWL_CACHE")
    if env:
        return Path(env).expanduser().resolve()
    return Path.home() / ".cdf" / "crawl"
...
def _is_csv(path: Path) -> bool:
    ...
    return s.endswith(".csv") or s.endswith(".cdf.csv")
...
def _looks_like_bdf_artifact(path: Path) -> bool:
    ...
    if sfx.endswith(".parquet") or sfx.endswith(".cdf.parquet"):
        return True
    if sfx.endswith(".feather") or sfx.endswith(".cdf.feather"):
        return True
    if sfx.endswith(".json") or sfx.endswith(".cdf.json"):
        return True
    # CSV: require either .cdf.csv OR BDF header row with required columns
    if _is_csv(path):
        if ".cdf.csv" in sfx:
            return True
...
# src/cdf/__init__.py (validate)
# src/cdf/__init__.py  (replace the existing validate with this)
...
def validate(
    ...
    *,
    ...
    def _bad_report(kind: str, detail: str, **extra):
        ...
        if raise_on_error:
            from .validate import CDFValidationError
            raise CDFValidationError(detail)
        return r
    ...
    if isinstance(obj, (str, Path)):
        ...
        def _looks_like_bdf_artifact(path: Path) -> bool:
            # quick filename hint: *.cdf.csv, *.cdf.parquet, *.cdf.feather, *.cdf.json(.gz)
            name_lc = path.name.lower()
            if any(name_lc.endswith(suf) for suf in (
                ".cdf.csv", ".cdf.csv.gz",
                ".cdf.parquet",
                ".cdf.feather",
                ".cdf.json", ".cdf.json.gz",
            )):
                return True
...
def plot(*args, **kwargs):
    ...
    Example:
        cdf.plot(df, xdata="Test Time / s", ydata="Voltage / V", yydata="Current / A",
    ...
    try:
        ...
        raise RuntimeError(
            "cdf.plot() requires the visualization module (matplotlib). "
...
def explore(*args, **kwargs):
    ...
    Example:
        cdf.explore(df, xdata="Test Time / s", ydata=["Voltage / V"], backend="plotly")
    """
    try:
        from ._explore import explore as _explore
    except Exception as e:
        raise RuntimeError("cdf.explore() is unavailable.") from e
...
def ingest(
    ...
    *,
    ...
    def _output_path(src: Path) -> Path:
        ...
        suffix = ".cdf.parquet" if fmt == "parquet" else ".cdf.csv"
    ...
    state_path = root / ".cdf.state.json"
    ...
    def _error_code(exc: Exception) -> str:
        ...
        if isinstance(exc, CDFValidationError):
        ...
        *, include_batteries: bool = False
        ...
        for f in sorted(bdf_files):
            ...
            if url:
                ...
                dataset_uri = f"cdf:dataset/{child_identifier}"
